...
def __read_6000():
    ...
    for i, subject in enumerate(tmpList):
        for task in sorted(os.listdir(os.path.join(repo_env.DATA_PATH_6000, subject))):
            ...
            drvDF = pd.read_csv(os.path.join(repo_env.DATA_PATH_6000, subject, task, subject + task + '-HostV_DrvInfo.csv'),
                                encoding='shift-jis', header=0,
                                names=['time', 'brake', 'gas', 'vel', 'steer', 'accX', 'accY', 'accZ', 'NaN'],
                                dtype='float16')
            drvDF = drvDF.drop(['time', 'NaN'], axis=1)

            roaDF = pd.read_csv(os.path.join(repo_env.DATA_PATH_6000, subject, task, subject + task + '-HostV_RoadInfo.csv'),
                                encoding='shift-jis', header=0,
